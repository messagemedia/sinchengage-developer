...
| **Method** | `GET` |
| **URL** | `https://eu.app.api.sinch.com/v2-preview/reporting/scheduled` |
| **Operation ID** | `GetActiveReport` |
...
|------|------|----------|-------------|-------------|
| `page_size` | number | No | Number of results to return in a page for paginated result sets. | Minimum: `1`; Maximum: `100` |
| `page_token` | string | No | Returned by Chronos service |  |
...
|--------|-------------|--------|
| 200 | A list of all messages received in the specified time window | `chronosscheduleresponse` |
| 401 | No valid authentication details were provided | None |
| 404 | Scheduled report not found. | None |

### 200 response schema (`chronosscheduleresponse`)
...
| Property | Type | Required | Description | Constraints |
|----------|------|----------|-------------|-------------|
| `label` | string | No | The label of the report schedule |  |
| `report` | object | No | A scheduled summary report request |  |
| `schedule` | object | No | The time schedule of a scheduled report |  |
| `scheduled_report_id` | string | No | The ID of the scheduled report. |  |
| `message_type` | string | No | The type of messages to include in the report. | Enum: `inbound`, `outbound`, `all` |
| `report_type` | string | No |  |  |
| `metadata` | object | No | Metadata for the scheduled report specified as a set of key value pairs, each key can be up to 100 characters long and each value can be up to 256 characters long. |  |
...
| `timezone` | string | Yes | The standard timezone name |  |
...
```bash
curl -X GET "https://eu.app.api.sinch.com/v2-preview/reporting/scheduled" \
...
```javascript
const response = await fetch("https://eu.app.api.sinch.com/v2-preview/reporting/scheduled", {
  "method": "GET",
  "headers": {
