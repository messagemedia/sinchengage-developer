...
| **Method** | `GET` |
| **URL** | `https://eu.app.api.sinch.com/api/v1/contacts/lists` |
| **Operation ID** | `getContactListsPage` |
| **Authentication** | Basic Auth, HMAC Auth |
| **Success** | `200` — Returns a contact lists page |
...
|------|------|----------|-------------|
| `request` | object | Yes | Pagination and filter options for the contact lists page (page tokens, page size, and related filters).<br> |
...
|--------|-------------|--------|
| 200 | Returns a contact lists page | `PageTokenDtoListData` |
...
| `content[].id` | string (uuid) | Yes | List id in UUID format |
| `content[].accountId` | string | Yes | Account id |
| `content[].vendorId` | string | Yes | Vendor id |
| `content[].name` | string | Yes | List name |
| `content[].alias` | string | Yes | List alias |
...
```bash
curl -X GET "https://eu.app.api.sinch.com/api/v1/contacts/lists?pageSize=1" \
...
```javascript
const response = await fetch("https://eu.app.api.sinch.com/api/v1/contacts/lists?pageSize=1", {
  method: "GET",
  headers: {
